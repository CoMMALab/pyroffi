#!/usr/bin/env bash
# Build _ik_jacobian_lib.so from _ik_jacobian_kernel.cu.
#
# Usage (from repo root):
#   bash build_kernels/build_ik_jacobian_cuda.sh
#   bash build_kernels/build_ik_jacobian_cuda.sh --debug
#
# Requirements:
#   - nvcc (CUDA toolkit)
...
SRC="${KERNELS_DIR}/ik/_ik_jacobian_kernel.cu"
OUT="${KERNELS_DIR}/ik/_ik_jacobian_lib.so"
...
# Override for a specific arch: GPU_ARCH=-arch=sm_80 bash build_ik_jacobian_cuda.sh
