...
def run(space="joint", n_steps=40, lr=s3.LR, n_iters=60, seed=0, n_restarts=1,
        out=None, spectrum=True):
    out = out or f"scratch/viz/loss_space_{space}.npz"
    ...
    if spectrum:
        ...
        _, _, r_tr = ident.select_rank(lam, s3.TRACE_FRAC, rule="trace")
