...
mkdir -p scratch/logs/multiseed_iosp scratch/viz/multiseed
...
run_job() {
  ...
  if [ "$stage" = "A" ]; then
    local log="scratch/logs/multiseed_iosp/A_${space}_seed${seed}.log"
    CUDA_VISIBLE_DEVICES="$gpu" python -u scratch/joint_loss_test.py \
      --space "$space" --seed "$seed" --no-spectrum \
      --out "scratch/viz/multiseed/A_${space}_seed${seed}.npz" > "$log" 2>&1
  else
    local log="scratch/logs/multiseed_iosp/B_seed${seed}.log"
    CUDA_VISIBLE_DEVICES="$gpu" python -u -m iosp.record_multistart_behavior \
      --space joint --seed "$seed" --n-branches 3 --n-starts 3 --steps 40 \
      --scene-b-scale 2.0 --no-render \
      --out "scratch/viz/multiseed/B_seed${seed}.npz" > "$log" 2>&1
  fi
