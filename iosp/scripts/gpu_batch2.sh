...
for spec in "$@"; do
    ...
    echo "[$(date +%H:%M:%S)] GPU=$GPU START $config/$run_name" | tee -a "$LOG_DIR/gpu${GPU}_queue.log"
    CUDA_VISIBLE_DEVICES=$GPU python -m iosp.run_experiment \
        "iosp/experiments/configs/$config" --run "$run_name" --gpu "$GPU" \
        > "$logf" 2>&1 || true
    rc=${PIPESTATUS[0]:-$?}
    echo "[$(date +%H:%M:%S)] GPU=$GPU DONE  $config/$run_name exit=$rc" | tee -a "$LOG_DIR/gpu${GPU}_queue.log"
done
echo "[$(date +%H:%M:%S)] GPU=$GPU ALL DONE" | tee -a "$LOG_DIR/gpu${GPU}_queue.log"
