...
Reduced parameterization (K=3, all in `iosp.pickplace`'s existing `transport`
...
not added to `iosp.pickplace`'s shared `SEGMENT_FEATURES`/`segment_
...
Usage:
    CUDA_VISIBLE_DEVICES=<idx> XLA_PYTHON_CLIENT_PREALLOCATE=false \\
        python -m iosp.study1_minimal_identifiable
"""
...
def _transport_residual_fn(problem):
    ...
    `iosp.pickplace`'s shared `SEGMENT_FEATURES`/`segment_residual_fn`, so
...
def theta_from_alpha_zero_prior(alpha, eigvecs, selected_idx):
...
def run_canonical(fit_scene_spec, held_out_specs=None, certificate_scene_specs=None,
    ...
    if alpha0_mode == "multistart":
        ...
        cma_alpha_hat, cma_trace = outer_opt.cma_es(loss, grid_alpha0, sigma0=sigma0, seed=seed, n_gens=n_steps)
    ...
    return dict(
        ...
        alpha_hat=np.asarray(alpha_hat), alpha0_mode=alpha0_mode, kkt_info=kkt_info,
...
def _print_canonical(r):
    ...
    print(f"fit RMSE (reconstruction of the FIT-scene demo):         {r['fit_rmse']:.4f}")
    print()
    print("held-out generalization RMSE (fitted vs no-fit baseline):")
...
if __name__ == "__main__":
    ...
    for mode, extra_kwargs in (
        ("multistart", dict(n_steps=40, n_starts=16)),
        ("cma", dict(n_steps=30)),
    ):
        print()
        print("=" * 60)
        print(f"CANONICAL PROCEDURE (deliberate demonstration theta_star, alpha0_mode={mode!r}, {extra_kwargs})")
        print("theta_star_demo =", np.asarray(theta_star_demo))
        print("=" * 60)
        r = run_canonical(
            CURATED_SCENES["clear"], prob=cert["prob"], forward_solver=cert["forward_solver"],
            theta_transport_star_override=theta_star_demo, alpha0_mode=mode, **extra_kwargs)
        _print_canonical(r)
