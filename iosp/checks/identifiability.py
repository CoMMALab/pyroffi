...
Theory: `iosp/THEORY_IDENTIFIABLE_REFIT.md` -- this script implements stage 2
(build G, eigendecompose) of the three-stage `fit wide -> Gram analysis ->
refit on the identifiable subspace` procedure derived there for the r < K
case.  It is a DIAGNOSTIC only: it reports the spectrum, it does not feed the
retained subspace `U_r` back into a refit (stage 4, still unimplemented).
...
Usage:
    CUDA_VISIBLE_DEVICES=<idx> XLA_PYTHON_CLIENT_PREALLOCATE=false \\
        python -m iosp.checks.identifiability
"""
