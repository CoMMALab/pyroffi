...
def _direct_lm_step_jax(
    ...
    *,
    ...
    When ``collision_fn`` is provided it is called as
    ``collision_fn(q, robot, collision_arg) -> scalar`` and its weighted
    residual ``sqrt(collision_weight) * penalty`` is appended to the LM
    residual vector so the solver actively steers away from obstacles.
    """
    ...
    sqrt_cw = jnp.sqrt(jnp.asarray(collision_weight, dtype=jnp.float32))
    ...
    def lm_single(cfg: Array, target_pos: Array) -> tuple[Array, Array]:
        ...
        def fused(q: Array) -> Array:
            ...
            if collision_fn is None:
                return base
            coll = collision_fn(q, robot, collision_arg)
            return jnp.append(base, sqrt_cw * coll)

        def lm_step(carry, _):
            c, lam, best_c, best_err = carry

            # Forward-mode Jacobian: n_act JVP passes, no backward intermediates stored.
            # Much lower peak memory than the equivalent vjp loop (n_out backward passes).
            f = fused(c)
            J = jax.jacfwd(fused)(c)   # (n_out, n_act)
            ...
            ori_err_r = jnp.linalg.norm(f[3:6] / W[3:])
            R = jnp.where(
...
def _svgd_region_step_jax(
    ...
    *,
    ...
    def pos_cost_one(q: Array, target: Array) -> Array:
        ...
        cost = pos_weight * jnp.sum((ee - target) ** 2)
        if collision_fn is not None:
            cost = cost + collision_weight * collision_fn(q, robot, collision_arg)
        return cost
