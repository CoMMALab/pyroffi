...
def _run_region_sampler_loop(
    *,
    ...
    """Shared multi-region collection loop used by the direct / JAX region-IK samplers.
    ...
    is ``None`` when ``sample_init_points=False``.

    Note: the brownian / SVGD / hit-and-run CUDA samplers in this module inline
    their own collection loops; this shared helper backs ``direct_*`` and the
    pure-JAX fallbacks in ``_region_ik_jax.py``.
    """
...
def direct_sample_box_region_cuda(
    ...
    *,
    ...
    seeds through the standard ``ls_ik_cuda`` Levenberg-Marquardt kernel.  The
    per-problem winner is the seed with the smallest weighted residual, and EE
    positions are recovered via a JAX FK pass on the winners.
    ...
    **Batched boxes**: pass ``box_min``/``box_max`` with shape ``(n_boxes, 3)``
    ...
    Args:
        ...
        collision_free: Enable in-kernel collision avoidance (requires
            ``collision_checker`` and ``collision_world``).
        collision_checker: Spherized robot collision model.
        collision_world: World geometry for collision avoidance.
        collision_weight: Weight on the collision residual term.
        collision_margin: Safety clearance [m].
