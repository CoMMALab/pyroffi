"""Sample Panda IK configurations inside a cartesian box region using SVGD — JAX fallback.

Pure-JAX implementation of Stein Variational Gradient Descent (SVGD) region IK,
analogous to the CUDA ``svgd_sample_box_region_cuda`` example (12_02) but
requiring no compiled CUDA kernel — runs on any JAX device (CPU or GPU via XLA).

Algorithm summary
-----------------
Particles start from random joint configurations.  Each SVGD iteration:

    phi(q_i) = (1/n) * sum_j [ K(q_i, q_j) * score(q_j)
                               + grad_{q_j} K(q_i, q_j) ]

where ``score(q) = -grad_q cost(q)`` (cost = pos_weight * ||fk(q) - target||^2)
and K is an RBF kernel with bandwidth ``h``.  The repulsion term (second summand)
spreads particles across the constraint manifold so samples cover the box region.

Run:
    python examples/12_05_box_region_ik_jax_svgd.py
    python examples/12_05_box_region_ik_jax_svgd.py --samples 64 --n-iters 30 --verbose
"""
...
from pyroffi.optimization_engines import svgd_sample_box_region_jax
...
def _stats_markdown(
    ...
    return (
        ...
        f"Mean position error: {err_mean:.6f}\n\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="JAX SVGD region IK sampler — no CUDA kernel required."
    )
    parser.add_argument("--samples", type=int, default=128,
                        help="IK samples to collect (lower default than CUDA example for CPU).")
    parser.add_argument("--seeds-per-launch", type=int, default=64)
    parser.add_argument("--restarts-per-target", type=int, default=8,
                        help="Number of SVGD particles per problem.")
    parser.add_argument("--n-iters", type=int, default=50,
                        help="SVGD iterations per launch.")
    parser.add_argument("--bandwidth", type=float, default=0.1,
                        help="RBF kernel bandwidth h (joint-space units).")
    parser.add_argument("--step-size", type=float, default=0.05,
                        help="SVGD gradient step size.")
    ...
    center_x = server.gui.add_slider("Box Center X", min=0.15, max=0.85, step=0.01,
                                     initial_value=float(default_box_center[0]))
    center_y = server.gui.add_slider("Box Center Y", min=-0.6, max=0.6, step=0.01,
                                     initial_value=float(default_box_center[1]))
    center_z = server.gui.add_slider("Box Center Z", min=0.05, max=0.9, step=0.01,
                                     initial_value=float(default_box_center[2]))
    size_x = server.gui.add_slider("Box Size X", min=0.02, max=0.8, step=0.01,
                                   initial_value=float(default_box_dims[0]))
    size_y = server.gui.add_slider("Box Size Y", min=0.02, max=0.8, step=0.01,
                                   initial_value=float(default_box_dims[1]))
    size_z = server.gui.add_slider("Box Size Z", min=0.02, max=0.8, step=0.01,
                                   initial_value=float(default_box_dims[2]))
    ...
    def _solve_now() -> None:
        ...
        _set_status("Solving (JAX SVGD)...")

        if solve_nonce == 0:
            rng_key_warmup = jax.random.PRNGKey(0)
            t0 = time.perf_counter()
            warm_cfgs, _, _, _ = svgd_sample_box_region_jax(
            ...
            print(f"JAX SVGD warmup (JIT compile + run): {warmup_ms:.1f} ms")

        solve_nonce += 1
        rng_key_run = jax.random.PRNGKey(solve_nonce)
        t0 = time.perf_counter()
        cfgs, ee_points, target_points, errors = svgd_sample_box_region_jax(
            rng_key=rng_key_run,
            box_min=box_min_jax,
            box_max=box_max_jax,
            **call_kwargs,
        ...
        print(f"JAX SVGD solve {solve_nonce}: {solve_ms:.1f} ms")
        ...
        server.scene.add_point_cloud(
            "/region/target_points", points=target_np, colors=target_colors,
            point_size=0.003, point_shape="sparkle",
        )
        server.scene.add_point_cloud(
            "/region/ee_points", points=ee_np, colors=ee_colors,
    ...
    last_box = np.concatenate([default_box_center, default_box_dims], axis=0)
    _draw_box()
    ...
    while True:
        ...
        if play.value and cfgs_np.shape[0] > 0:
